...
export async function GET(req: NextRequest): Promise<NextResponse> {
    ...
    if(!origin || !id_origin) return NextResponse.json({ error: "No origin provided" }, { status: 400 });

    const value:Object = dictionaryOrigin[origin];

    if(!value) return NextResponse.json({ error: "Origin not found" }, { status: 404 });
    ...
    if(!originData) return NextResponse.json({ error: "Origin not found" }, { status: 404 });
